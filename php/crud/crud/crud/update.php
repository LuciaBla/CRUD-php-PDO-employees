...
if ($_GET["IDm"]!=""){//que no este vacio
    ...
    $aux =fopen("aux.txt", "w") or die("Unable to open file!");//Abrir un archivo temporal 
    //aux.txt en modo de escritura
    while(!feof($myfile)) {//lee linia por linia
        $record = fgets($myfile);//lee linia
        $fields = explode ("#", $record);//divide la liniea
        if ($fields[0]==$_GET["IDm"]){//Si el primer campo ($fields[0]) coincide con IDm, 
            //escribe la nueva línea en aux.txt. Si no, escribe la línea original.
...
if ($_GET["ID"]!=""){//que no este vacio
    $myfile = fopen("users.txt", "r") or die("Unable to open file!");//Abrir el archivo 
    //users.txt en modo de lectura y buscar el registro
    while(!feof($myfile)) {
        $record = fgets($myfile);
    ...
    fclose ($myfile);
    if (isset($id_founded)) { //Si se encuentra el registro, mostrar el 
